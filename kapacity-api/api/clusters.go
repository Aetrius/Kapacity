...
func setupClusterRoutes(nodes *gin.RouterGroup, db gorm.DB, environmentIn string) {
...
func getCluster(c *gin.Context, db gorm.DB) {
	clusterID := c.Param("id")
	c.JSON(http.StatusOK, gin.H{"message": "Get Cluster", "clusterID": clusterID})
}

func getClusters(c *gin.Context, db gorm.DB, environmentIn string) {
	...
	getPaginatedData(c, db, &KapacityAPI.Cluster{}, pageStr, pageSizeStr, additionalParams, sortColumn, orderBy)
}
